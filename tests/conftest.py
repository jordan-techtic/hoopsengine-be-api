...
def unverified_player_user(password_hashes: dict[str, str]) -> User:
    ...
    now = datetime.now(timezone.utc)
    with Session(sync_engine) as session:
        existing = session.get(User, UNVERIFIED_PLAYER_ID)
        if existing is None:
            user = User(
                id=UNVERIFIED_PLAYER_ID,
                email=UNVERIFIED_PLAYER_EMAIL,
                username="unverifiedplayer",
                encrypted_password=hash_password(UNVERIFIED_PLAYER_PASSWORD),
                role=UserRole.PLAYER.value,
                first_name="Unverified",
                last_name="Player",
                is_super_admin=False,
        ...
        existing.email = UNVERIFIED_PLAYER_EMAIL
        existing.username = "unverifiedplayer"
        existing.encrypted_password = hash_password(UNVERIFIED_PLAYER_PASSWORD)
        existing.role = UserRole.PLAYER.value
        existing.email_confirmed_at = None
        existing.confirmation_token = hash_otp(TEST_OTP_CODE)
        existing.confirmation_sent_at = now
        existing.is_active = True
...
@pytest.fixture
def unverified_player_headers(unverified_player_user: User) -> dict[str, str]:
    """JWT auth header for the unverified player fixture."""
    token = create_access_token(
        unverified_player_user.id,
        extra_claims={"email": unverified_player_user.email, "role": unverified_player_user.role},
    )
    return auth_headers(token)
